 ...
 contract BhkToken is IBEP20
{
    ...
    bool public _isburning;
    bool public _ispair;
    address public lapan; //拉盘合约地址
    address public jsaddr;
    ...
    {
        _owner = msg.sender;
        _name="Black Hole King";
        _symbol="BHK";
        _decimals = 8;
        _minter[msg.sender]=true;
        _mint(_owner,630000*1e8);
        _isburning=true;
        ...
        jsaddr = 0x71746703C1c8F7f78a5594CF33ebF375b79251Db;  //技术滑点
        backaddr = 0xEF86664239d56A880Af8b495e4c2870346046Bbe; //运营滑点
    }
    ...
    function setjsaddr(address _jsaddr) public
    {
        require(msg.sender==_owner);
        jsaddr=_jsaddr;
    }
    function setlapan(address _lapan) public
    {
        require(msg.sender==_owner);
        lapan=_lapan;
    }
    ...
    function takeOutErrorTransfer(address tokenaddress, address addr) public
    {
        require(msg.sender==_owner);
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(_isburning && (sender== uniswapV2Pair && !_minter[recipient]))
        {
            revert("Can not buy");
        }
        ...
        if(_ispair && recipient == uniswapV2Pair && sender != address(this) && sender != lapan) {
            //计算滑点并扣除
            uint _hua = amount * 17 / 100;
            uint real = amount.sub(_hua);
            _tokenTransfer(sender, address(this), _hua);

            //授权薄饼并声明交易路径
            _approve(address(this), router_address, _hua);
            uint deadline = block.timestamp + 15;
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = usdt;
            //USDT滑点
            uniswapV2Router.swapExactTokensForTokens(_hua, 0, path, address(cson), deadline);
            cson.withdrawal(address(this));
            uint _usdtyu = IERC20(usdt).balanceOf(address(this));
            //按比例分配
            uint jsamount = _usdtyu * 1 / 17;
            IERC20(usdt).transfer(jsaddr, jsamount); //技术滑点分红

            uint backamount = _usdtyu * 3 / 17;
            IERC20(usdt).transfer(backaddr, backamount); //运营滑点分红

            uint lpusdt = IERC20(usdt).balanceOf(address(this));
            IERC20(usdt).transfer(lapan, lpusdt); //拉盘合约
            _tokenTransfer(sender, recipient, real);
        }else{
...
contract son is OwnAdmin{
    ...
    //提币
    function withdrawal(address recipient) public onlyOwner
    {
        uint _usdtyu = IERC20(usdt).balanceOf(address(this));
      
        IERC20(usdt).transfer(recipient, _usdtyu);
    }
